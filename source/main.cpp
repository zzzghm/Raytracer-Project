...
bool shadowFeeler(vec4 p0, Object *object){
  ...
  std::vector < Object::IntersectionValues> intersections(sceneObjects.size());
  for (int i = 6; i < 8; i++) {

	  intersections[i] = sceneObjects[i]->intersect(p0, normalize(lightPosition-p0));
	
  }
  if (intersections[6].t_w > 0.01 || intersections[7].t_w > 0.01){
  if (intersections[6].t_w != std::numeric_limits< double >::infinity() || intersections[7].t_w != std::numeric_limits< double >::infinity()) {
...
// add four more point light source at the neighbors
bool shadowFeeler1(vec4 p0, Object *object) {
	bool inShadow = false;

	std::vector < Object::IntersectionValues> intersections(sceneObjects.size());
	vec4 lightPosition[4] = { vec4(0.3,1.5,0,1),
							  vec4(-0.3,1.5,0,1),
							  vec4(0,1.5,0.3,1),
							  vec4(0,1.5,-0.3,1) };
	for (int j = 0; j < 4; j++) {
		for (int i = 6; i < 8; i++) {
			...
			intersections[i] = sceneObjects[i]->intersect(p0, normalize(lightPosition[j] - p0));
...
// add four more point light source at the diagonal neighbors
bool shadowFeeler2(vec4 p0, Object *object) {
	bool inShadow = false;

	std::vector < Object::IntersectionValues> intersections(sceneObjects.size());
	vec4 lightPosition[4] = { vec4(-0.3,1.5,0.3,1),
		                      vec4(0.3,1.5,0.3,1),
		                      vec4(-0.3,1.5,-0.3,1),
		                      vec4(0.3,1.5,-0.3,1) };
	for (int j = 0; j < 4; j++) {
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
 ...
 std::vector < Object::IntersectionValues> intersections(sceneObjects.size());
  ...
  for (int i = 0; i < sceneObjects.size(); i++) {
	  intersections[i] = sceneObjects[i]->intersect(p0, E);
  ...
  if (intersections[0].t_w != std::numeric_limits<double>::infinity()) {
      Object *int_object = sceneObjects[intersections[0].ID_];
	  vec4 L = normalize(lightPosition - intersections[0].P_w);
	  vec4 N = normalize(intersections[0].N_w);
	  vec4 V = normalize(E);
	  // vec4 R = normalize(2 * N*dot(N, L) - L); R = -R;
	  vec4 R = normalize(reflect(L, N));
	  vec4 H = normalize(L + V);
      double Shininess = int_object->shadingValues.Kn;

	  double Ka = 1;
	  double Kd = fmax(0, dot(L, N));
	  double Ks = pow(max(dot(V, R), 0.0), Shininess);

      vec4 ambient = (int_object->shadingValues.color)*(int_object->shadingValues.Ka)*lightColor;
	  ambient.w = 1.0;
	  vec4 diffuse = (int_object->shadingValues.color)*lightColor;
	  diffuse.w = 1.0;
	  vec4 specular = (int_object->shadingValues.color)*(int_object->shadingValues.Ks)*lightColor;
	  specular.w = 1.0;
	  color = Ka * ambient + Kd * diffuse + Ks * specular;

   if (intersections[0].ID_ != 4 && intersections[0].ID_ != 6 && intersections[0].ID_ != 7) {
			   bool inShadow = shadowFeeler(intersections[0].P_w, int_object);
			   bool inShadow1 = shadowFeeler1(intersections[0].P_w, int_object);
			   bool inShadow2 = shadowFeeler2(intersections[0].P_w, int_object);
			   if (inShadow)
			   {
				   color = Ka * ambient;
				   color.w = 1;
  ...
  if (sceneObjects[intersections[0].ID_]->shadingValues.Ks > 0) {
	  depth = depth + 1;
	  vec4 N_r = normalize(intersections[0].N_w);
	  vec4 R_i = normalize(E);
	  vec4 R_r = normalize(reflect(R_i, N_r));
	 // vec4 E_r = normalize(E_i - 2 * N_r*dot(E_i, N_r));
	 // if (normalize(R_i) = normalize(N_r)) { R_r = N_r; }
	 // if (dot(-R_i, N_r) == 0) { R_r = R_i; }
	  color = castRay(intersections[0].P_w, R_r, NULL, depth);
  }

  if (sceneObjects[intersections[0].ID_]->shadingValues.Kr > 0) {
	  depth = depth + 1;
	  vec4 N_r = normalize(intersections[0].N_w);
	  vec4 R_i = normalize(E);
	  vec4 R_r = normalize(reflect(R_i, N_r));
	  ...
	  //if (normalize(R_i) = normalize(N_r)) { R_r = N_r; }
	  //if (dot(-R_i, N_r) == 0) { R_r = R_i; }
	  vec4 color_reflect = castRay(intersections[0].P_w, R_r, NULL, depth);

	  double costheta1 = -fmax(0, dot(R_i, N_r));
	  double sintheta1 = sqrt(1 - fmax(0, pow(costheta1, 2)));
	  double sintheta2 = sintheta1 / sceneObjects[intersections[0].ID_]->shadingValues.Kr;
	  double costheta2 = sqrt(1-fmax(0,pow(sintheta2,2)));
	  double index1 = 1 / sceneObjects[intersections[0].ID_]->shadingValues.Kr;
	  
	  vec4 T1 = normalize((-index1 * costheta1 - sqrt(1 - pow(index1, 2)*pow(sintheta1, 2)))*N_r + index1 * R_i); T1.w = 0;
	  
	  Object::IntersectionValues intersect_re;
	  intersect_re = sceneObjects[6]->intersect(intersections[0].P_w,T1);

	  vec4 N_rr = normalize(intersect_re.N_w); N_rr.w = 0;
	  double costheta3 = fmax(0, dot(T1,N_rr));
	  double sintheta3 = sqrt(1-fmax(0,pow(costheta3,2)));
	  double sintheta4 = sintheta3* sceneObjects[intersections[0].ID_]->shadingValues.Kr;
	  double costheta4 = sqrt(1 - fmax(0, pow(sintheta4, 2)));
	  double index2= sceneObjects[intersections[0].ID_]->shadingValues.Kr;
	  vec4 T2 = normalize((-index2 * costheta2 - sqrt(1 - pow(index2, 2)*pow(sintheta2, 2)))*N_rr + index2 * T1); T2.w = 0;
	 // color = castRay(intersect_re.P_w, T2, sceneObjects[intersections[0].ID_], depth);
	  vec4 color_transmit = castRay(intersect_re.P_w, T1, sceneObjects[intersections[0].ID_], depth);

	  // Fresnel equations with reflection and transmission
	  double n1 = sceneObjects[intersections[0].ID_]->shadingValues.Kr;
	  double n2= 1;
	  double R_v = pow((n1*costheta1 - n2 * costheta2),2) / pow((n1*costheta1 + n2 * costheta2),2);
	  double R_p= pow((n2*costheta1 - n1 * costheta2),2) / pow((n2*costheta1 + n1 * costheta2), 2);
	  double R = 0.02*(R_v+R_p);
	  double T = 1 - R;

	  color = R * color_reflect + T * color_transmit;
	  //color = T*color_transmit;
...
void initCornellBox(){
  ...
  { //Back Wall
    ...
    _shadingValues.color = vec4(0.75, 0.75, 0.75,1.0);
...
void initUnitSphere(){
  ...
  Object::ShadingValues _shadingValues;
  _shadingValues.color = vec4(1.0,0.0,0.0,1.0);//red
  _shadingValues.Ka = 0.0;
  _shadingValues.Kd = 1.0;
  _shadingValues.Ks = 0.0;
  _shadingValues.Kn = 16.0;
 ...
 // sceneObjects[sceneObjects.size()-1]->setModelView(mat4());
  sceneObjects[sceneObjects.size() - 1]->setModelView(mat4()*Translate(1, 0, 0)*Scale(0.5, 0.5, 0.5));
  }
  
  {
	  sceneObjects.push_back(new Sphere("Diffuse sphere"));
	  Object::ShadingValues _shadingValues;
	  _shadingValues.color = vec4(0.0, 1.0, 0.0, 1.0);//green
	  _shadingValues.Ka = 0.0;
	  _shadingValues.Kd = 1.0;
	  _shadingValues.Ks = 0.0;
	  _shadingValues.Kn = 16.0;
	  ...
	  sceneObjects[sceneObjects.size() - 1]->setShadingValues(_shadingValues);
	  // sceneObjects[sceneObjects.size()-1]->setModelView(mat4());
	  sceneObjects[sceneObjects.size() - 1]->setModelView(mat4()*Translate(-1,0,0));
  }
}
...
void initUnitSquare(){
  ...
  { //Back Wall
    ...
    _shadingValues.Kr = 0.0;
    sceneObjects[sceneObjects.size()-1]->setShadingValues(_shadingValues);
    sceneObjects[sceneObjects.size()-1]->setModelView(mat4()*Translate(-1, 0, 0)*RotateZ(45)*Scale(0.5,0.5,0.5));
  }
